...
#ifdef OPENVPN_EXTERNAL_TUN_FACTORY
    // Double-check androidClient is valid (defensive programming against segfaults)
    if (!session->androidClient) {
        LOGE("openvpn_wrapper_get_app_fd: androidClient not initialized");
        return -1;
    }
    
    // Additional safety check: verify androidClient pointer is valid before dereferencing
    // This helps catch cases where the pointer was corrupted or the object was deleted
    try {
        ...
        LOGE("openvpn_wrapper_get_app_fd: Exception calling getAppFd(): %s", e.what());
        return -1;
    } catch (...) {
        LOGE("openvpn_wrapper_get_app_fd: Unknown exception calling getAppFd()");
        return -1;
    }
#else
