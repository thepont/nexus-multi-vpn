            ...
            try {
                ...
                if (authFilePath != null) {
                    ...
                    try {
                        ...
                        // Hardening: Delete temporary plaintext credentials file immediately once read into memory
                        try {
                            if (authFile.exists()) {
                                authFile.delete()
                                Log.i(TAG, "🔒 Plaintext temporary credentials file deleted immediately from cache: $authFilePath")
                            }
                        } catch (e: Exception) {
                            Log.w(TAG, "Warning: Failed to delete temporary credentials file: ${e.message}")
                        }
                    }
                } else {
