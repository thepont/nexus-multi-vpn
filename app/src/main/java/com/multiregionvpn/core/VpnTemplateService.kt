...
class VpnTemplateService @Inject constructor(
    ...
    private suspend fun prepareNordVpnConfig(config: VpnConfig): PreparedVpnConfig {
        ...
        // 3. DO NOT write credentials to disk. Pass them in memory.
        
        // 4. Modify the .ovpn config string
        val modifiedConfig = baseConfig
            // Find the default "auth-user-pass" line and replace with just "auth-user-pass"
            // This indicates to OpenVPN 3 that credentials are required
            .replace(
                "auth-user-pass",
                "auth-user-pass"
            )
