...
class VpnTemplateService @Inject constructor(
    ...
    private suspend fun prepareNordVpnConfig(config: VpnConfig): PreparedVpnConfig {
        ...
        withContext(Dispatchers.IO) {
            ...
            // SECURITY: Restrict file permissions to owner-only (read/write) to prevent unauthorized read access to credentials on disk
    ...
    private suspend fun prepareLocalTestConfig(config: VpnConfig): PreparedVpnConfig {
        ...
        withContext(Dispatchers.IO) {
            ...
            // SECURITY: Restrict file permissions to owner-only to protect test credentials
            authFile.setReadable(true, true)
            authFile.setWritable(true, true)
            authFile.setExecutable(false, false)
